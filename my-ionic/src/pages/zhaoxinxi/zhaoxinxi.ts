...
@IonicPage()
@Component({
  selector: 'page-zhaoxinxi',
  templateUrl: 'zhaoxinxi.html',
})
export class ZhaoxinxiPage {
  name:any;
  iSrc = "assets/imgs/gouxuan.png";
  index:any;
  idx:any = 1;
  id:any = 'msg';
  xinxiArr = ['资产包转让','债权转让','固产转让','商业保理','典当担保','融资借贷','悬赏信息','尽职调查','资产求购','委外催收','法律服务'];
  gengduo = ['抵押','信用','综合'];
  diquArr = ['安徽','澳门','北京','福建','甘肃','广东','广西','贵州','海南','河北','河南','黑龙江','湖北','湖南','吉林','江苏','江西','辽宁','内蒙古','宁夏','青海','山东','山西','陕西','上海','四川','台湾','天津','西藏','香港','新疆','云南','浙江','重庆','海外'];
  diquArr2 = [
  ...
  FontChange(){
    ...
    $('#'+this.id).on('click','ion-item',(e)=>{
      let nowclick = $(e.currentTarget);
      let displays = $(e.currentTarget).find('img').attr('src') === ""?this.iSrc:"";
      nowclick.find('img').attr('src',displays);
      nowclick.find('span').css('color','#fedb47');
      if((lastclicked !== null) && (lastclicked.get(0) !== nowclick.get(0))){
        lastclicked.find('img').attr('src',"");
        lastclicked.find('span').css('color','black');
      }
      lastclicked = nowclick;
    })
  }
  ionViewDidLoad(){
    // 头部折叠栏
    let $lastclicked=$('.zhaoxinxi_list').find('ion-label');
    $('.zhaoxinxi_list').on('click','ion-label',(e)=>{
      let $nowclick = $(e.currentTarget);
      let display = $nowclick.find('img').attr('src') === "assets/imgs/arrow_down.png"?"assets/imgs/arrow_up.png":"assets/imgs/arrow_down.png"
      $nowclick.find('img').attr('src',display);
      $nowclick.find('b').css('color','#fedb47');
      if(($lastclicked !== null) && ($lastclicked.get(0) !== $nowclick.get(0))){
        $lastclicked.find('img').attr('src',"assets/imgs/arrow_down.png");
        $lastclicked.find('b').css('color','black');
        $nowclick.find('img').attr('src',"assets/imgs/arrow_up.png");
        $nowclick.find('b').css('color','#fedb47');
      }
      $lastclicked = $nowclick;
    })
  ...
  diqu_item(){
    var lastclick = null;
    $('.diqu_list').on('click','ion-item',(e)=>{
      let nowclick = $(e.currentTarget);
      let color = nowclick.find('ion-label').css('color') == '#fedb47' ? 'black': '#fedb47';
      nowclick.find('ion-label').css('color',color);
      if((lastclick !== null) && (lastclick.get(0) !== nowclick.get(0))){
        lastclick.find('ion-label').css('color','black');
      }
      lastclick = nowclick;
      var str = nowclick.find('ion-label')[0].innerText;
      var province = this.diquArr.indexOf(str);
      this.provinceArr = this.diquArr2[province];
    });

    var lastclick2 = null;
    $('.diqu2_list').on('click','ion-item',(e)=>{
      let nowclick = $(e.currentTarget);
      let color = nowclick.find('ion-label').css('color') == '#fedb47' ? 'black': '#fedb47';
      nowclick.find('ion-label').css('color',color);
      if((lastclick2 !== null) && (lastclick2.get(0) !== nowclick.get(0))){
        lastclick2.find('ion-label').css('color','black');
      }
      lastclick2 = nowclick;
    });
  }
